...
function billingType(inv: Invoice, isFirst: boolean): { label: string; pill: string } {
  if (isFirst) return { label: "New", pill: "bg-green-50 text-green-700 border border-green-100" };
  if (inv.billing_reason === "subscription_update") return { label: "Upgrade", pill: "bg-purple-50 text-purple-700 border border-purple-100" };
  return { label: "Renewal", pill: "bg-blue-50 text-blue-600 border border-blue-100" };
}
...
export default function CustomerDetailPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto py-8 px-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-7 animate-pulse flex gap-5 items-start">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-200 p-5 animate-pulse space-y-2">
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto py-8 px-6">
        <a href="/customers" className="text-xs text-gray-400 hover:text-gray-700 transition-colors">← Customers</a>
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-6 space-y-5">
      ...
      {/* Back */}
      <a href="/customers" className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-700 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-7">
        <div className="flex items-start gap-5">
          ...
          <div className={`w-14 h-14 rounded-full flex items-center justify-center text-xl font-bold shrink-0 ${
            isChurned ? "bg-red-100 text-red-500" : "bg-indigo-100 text-indigo-600"
          }`}>
            {initial}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5 flex-wrap">
              <h1 className="text-xl font-semibold text-gray-900 truncate">{customerEmail}</h1>
              {isChurned ? (
                <span className="text-xs bg-red-50 text-red-600 border border-red-100 px-2.5 py-0.5 rounded-full shrink-0">Churned</span>
              ) : activeSub ? (
                <span className="text-xs bg-green-50 text-green-700 border border-green-100 px-2.5 py-0.5 rounded-full shrink-0">Active</span>
              ) : (
                <span className="text-xs bg-gray-100 text-gray-500 px-2.5 py-0.5 rounded-full shrink-0">No subscription</span>
              )}
            </div>
            ...
            <div className="mt-3">
              {editingNote ? (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setEditingNote(false); setNoteInput(""); }} className="text-xs text-gray-400 px-2">✕</button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  {note && <p className="text-xs text-gray-500 bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5 italic">"{note}"</p>}
                  <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
          <p className="text-xs font-semibold text-gray-500 mb-2">Total Revenue</p>
          <p className="text-2xl font-bold text-gray-900">£{(total / 100).toLocaleString("en-GB", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          {paidInvoices.length > 0 && <p className="text-xs text-gray-400 mt-0.5">{paidInvoices.length} payment{paidInvoices.length !== 1 ? "s" : ""}</p>}
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
          <p className="text-xs font-semibold text-gray-500 mb-2">Avg Payment</p>
          <p className="text-2xl font-bold text-gray-900">£{(avgPayment / 100).toFixed(2)}</p>
          {lastPayment && <p className="text-xs text-gray-400 mt-0.5">Last {lastPayment.toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</p>}
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
          <p className="text-xs font-semibold text-gray-500 mb-2">Tenure</p>
          <p className="text-2xl font-bold text-gray-900">{tenureMonths > 0 ? `${tenureMonths}mo` : "—"}</p>
          {firstPayment && <p className="text-xs text-gray-400 mt-0.5">since {firstPayment.toLocaleDateString("en-GB", { month: "short", year: "numeric" })}</p>}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-6 py-3 border-b border-gray-100 bg-gray-50/60">
          ...
          <p className="text-xs font-semibold text-gray-500 text-right">Amount</p>
          <p className="text-xs font-semibold text-gray-500 w-4" />
        </div>

        {displayInvoices.length === 0 ? (
          <p className="px-6 py-10 text-sm text-gray-400 text-center">No invoices found.</p>
        ) : (
          <ul>
            {displayInvoices.map(({ inv, type }) => (
              <li key={inv.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 items-center px-6 py-3.5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                ...
                <span className={`text-sm font-semibold tabular-nums text-right ${inv.amount_paid > 0 ? "text-gray-800" : "text-gray-400"}`}>
