...
export default function Home() {
  ...
  const [recoveryStats, setRecoveryStats] = useState<{ totalRecovered: number; recoveredThisMonth: number; totalRecoveredCount: number; totalSent: number } | null>(null);
  const [expansionCandidates, setExpansionCandidates] = useState<{ id: string; email: string; planName: string; mrr: number; monthsAsCustomer: number; totalPaid: number }[]>([]);
  const [upgradeDrafts, setUpgradeDrafts] = useState<Record<string, { content: string; loading: boolean; open: boolean }>>({});
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) return;
      fetch("/api/recovery-stats", { headers: { Authorization: `Bearer ${session.access_token}` } })
        .then(r => r.json())
        .then(d => { if (!d.error) setRecoveryStats(d); })
        .catch(() => {});
    });
  }, []);
  ...
  async function sendRecoveryEmail(customerId: string, email: string, body: string) {
    ...
    try {
      ...
      const res = await fetch("/api/send-recovery-email", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` },
        body: JSON.stringify({ customerEmail: email, subject: "Just checking in", body }),
      });
      if (res.ok) setEmailSent(prev => ({ ...prev, [customerId]: true }));
    } catch {}
    setSending(prev => ({ ...prev, [customerId]: false }));
  }

  async function draftUpgrade(c: { id: string; email: string; planName: string; mrr: number; monthsAsCustomer: number; totalPaid: number }) {
    setUpgradeDrafts(prev => ({ ...prev, [c.id]: { content: "", loading: true, open: true } }));
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch("/api/draft-upgrade", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` },
        body: JSON.stringify({ email: c.email, planName: c.planName, mrr: c.mrr, monthsAsCustomer: c.monthsAsCustomer, totalPaid: c.totalPaid }),
      });
      if (!res.ok || !res.body) { setUpgradeDrafts(prev => ({ ...prev, [c.id]: { content: "Failed to generate email.", loading: false, open: true } })); return; }
      setUpgradeDrafts(prev => ({ ...prev, [c.id]: { ...prev[c.id], loading: false } }));
      const reader = res.body.getReader();
      const decoder = new TextDecoder();
      while (true) {
        const { done, value } = await reader.read();
        if (done) break;
        setUpgradeDrafts(prev => ({ ...prev, [c.id]: { ...prev[c.id], content: prev[c.id].content + decoder.decode(value, { stream: true }) } }));
      }
    } catch {
      setUpgradeDrafts(prev => ({ ...prev, [c.id]: { content: "Failed to generate email.", loading: false, open: true } }));
    }
  }
  ...
  function processData(data: any, selectedRange: Range) {
    ...
    // Expansion candidates — loyal customers on non-top plans
    const atRiskIdSet = new Set((data.atRiskSubscriptions || []).map((c: any) => c.id));
    const maxPlanMrr = Math.max(...Object.values(data.mrrByCustomer || {}).map((c: any) => c.mrr), 0);
    const candidates = (data.customers || [])
      .filter((c: any) =>
        !c.churned &&
        !atRiskIdSet.has(c.id) &&
        c.payments >= 6 &&
        data.mrrByCustomer?.[c.id] &&
        data.mrrByCustomer[c.id].mrr < maxPlanMrr * 0.85
      )
      .map((c: any) => ({
        id: c.id,
        email: c.email,
        planName: data.mrrByCustomer[c.id].planName,
        mrr: data.mrrByCustomer[c.id].mrr,
        monthsAsCustomer: Math.round(c.payments),
        totalPaid: c.total,
      }))
      .sort((a: any, b: any) => b.monthsAsCustomer - a.monthsAsCustomer)
      .slice(0, 3);
                  ...
                  <div key={c.id} className="bg-indigo-50 border border-indigo-100 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-3.5">
                      ...
                      <div className="shrink-0 flex items-center gap-3 ml-4">
                        <button
                          onClick={() => {
                            if (draft?.open) setUpgradeDrafts(prev => ({ ...prev, [c.id]: { ...prev[c.id], open: false } }));
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        ...
        <ul className="space-y-0">
          {events.slice(0, 20).map((event, i) => {
            ...
            const initial = (event.email || "?")[0].toUpperCase();
            const href = event.customerId ? `/customers/${event.customerId}` : null;
            const Row = (
              <>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${cfg.avatar}`}>
                  {initial}
                </div>
